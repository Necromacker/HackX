        ...
        <div className="pos-billing-layout">
          ...
          <div className="pos-scanner-column">
            <div className="pos-card">
              <div className="pos-card-header">
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <ScanLine size={20} color="var(--foody-orange)" />
                  <h3 style={{ fontSize: '1.05rem', fontWeight: 800 }}>Scan Product Barcode</h3>
                </div>
                <div className="scanner-mode-switch-small">
                  <button className={`mode-btn-sm ${scannerMode === 'upload' ? 'active' : ''}`} onClick={() => { setScannerMode('upload'); stopCamera(); }}>
                    <Upload size={14} /> Upload Image
                  </button>
                  <button className={`mode-btn-sm ${scannerMode === 'camera' ? 'active' : ''}`} onClick={() => setScannerMode('camera')}>
              ...
              {scannerMode === 'upload' && (
                <>
                  ...
                  />
                  <div className="billing-upload-dropzone"
                  ...
                  >
                    {isScanning ? (
                      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px' }}>
                        <RefreshCw className="spinner" size={32} color="var(--foody-orange)" />
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '6px' }}>
                        ...
                        <span style={{ fontSize: '0.8rem', color: 'var(--foody-orange)', fontWeight: 700 }}>
                          Image Analyzed - Click or drop another image
                        </span>
                      </div>
                    ) : (
                      <>
                        <div className="billing-upload-icon"><ImageIcon size={26} /></div>
              ...
              {scannerMode === 'camera' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <button onClick={captureAndScanCameraFrame} className="btn-camera-snap">
              ...
              <div style={{ marginTop: '14px', paddingTop: '14px', borderTop: '1px solid var(--border-light)' }}>
                ...
                <form onSubmit={(e) => { e.preventDefault(); lookupAndAddBarcode(manualBarcodeInput); }} style={{ display: 'flex', gap: '8px' }}>
                  <input type="text" placeholder="e.g. 8901262153362" className="billing-barcode-input" value={manualBarcodeInput} onChange={(e) => setManualBarcodeInput(e.target.value)} />
                  <button type="submit" className="btn-add-barcode">+ Add to Bill</button>
                </form>
              </div>
            </div>
            ...
            <div className="pos-card" style={{ marginTop: '16px' }}>
              <h4 style={{ fontSize: '0.88rem', fontWeight: 800, color: 'var(--text-main)', marginBottom: '10px', display: 'flex', alignItems: 'center', gap: 6 }}>
                <Star size={14} color="var(--foody-orange)" /> Quick Add Verified Products
              </h4>
              <div className="quick-add-grid">
                {products.filter(p => [95, 201, 202, 203, 1, 21].includes(p.product_id)).map(p => (
                  <button key={p.product_id} className="quick-item-btn" onClick={() => addProductToBill(p)}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                      <div className="quick-item-icon"><Package size={16} /></div>
                      <div style={{ textAlign: 'left' }}>
                        <div style={{ fontSize: '0.82rem', fontWeight: 700, color: 'var(--text-main)', lineHeight: 1.2 }}>{p.product_name}</div>
                        <div style={{ fontSize: '0.72rem', color: 'var(--text-sub)' }}>Rs.{p.unit_price} | {p.stock_level} in stock</div>
                      </div>
                    </div>
                    <Plus size={16} color="var(--foody-orange)" />
                  </button>
                ))}
              </div>
            </div>
          ...
          <div className="pos-terminal-column">
            <div className="pos-card pos-bill-terminal">
              <div className="terminal-header">
                <div>
                  <h3 style={{ fontSize: '1.15rem', fontWeight: 800, color: 'var(--foody-orange)' }}>Current Active Bill</h3>
                  <p style={{ fontSize: '0.78rem', color: 'var(--text-sub)' }}>Items scanned: {billItems.reduce((s, i) => s + i.quantity, 0)} Units</p>
                </div>
                {billItems.length > 0 && (
                  <button onClick={() => setBillItems([])} className="btn-clear-bill" title="Clear bill">
              ...
              <div className="customer-info-row">
                <div style={{ flex: 1 }}>
                  ...
                  <input type="text" className="customer-input" value={customerName} onChange={(e) => setCustomerName(e.target.value)} placeholder="Customer Name" />
                </div>
                <div style={{ flex: 1 }}>
                  <label className="customer-label">Phone Number</label>
                  <input type="text" className="customer-input" value={customerPhone} onChange={(e) => setCustomerPhone(e.target.value)} placeholder="e.g. 9876543210" />
                </div>
              </div>
              ...
              <div className="bill-table-container">
                {billItems.length === 0 ? (
                  <div className="empty-bill-state">
                    ...
                    <p style={{ fontWeight: 700, color: 'var(--text-main)', marginTop: '8px' }}>No items scanned in bill</p>
                    <p style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>Upload a product image or scan a barcode to add items.</p>
                  </div>
                ) : (
                    ...
                    <tbody>
                      {billItems.map(item => (
                        <tr key={item.product_id}>
                          <td>
                            <div style={{ fontWeight: 700, fontSize: '0.88rem', color: 'var(--text-main)' }}>{item.product_name}</div>
                            <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)' }}>
                              ID #{item.product_id} {item.barcode && `| Barcode: ${item.barcode}`}
                            </div>
                          </td>
                          <td style={{ textAlign: 'center', fontWeight: 600, fontSize: '0.88rem' }}>Rs.{item.unit_price.toFixed(0)}</td>
                          <td style={{ textAlign: 'center' }}>
                            <div className="qty-controls">
                              <button onClick={() => updateQuantity(item.product_id, -1)} className="qty-btn"><Minus size={12} /></button>
                              <span style={{ fontWeight: 800, fontSize: '0.9rem', minWidth: '18px', textAlign: 'center' }}>{item.quantity}</span>
                              <button onClick={() => updateQuantity(item.product_id, 1)} className="qty-btn"><Plus size={12} /></button>
                            </div>
                          </td>
                          ...
                          <td style={{ textAlign: 'center' }}>
                            <button onClick={() => removeBillItem(item.product_id)} className="btn-delete-item"><Trash2 size={14} /></button>
                          </td>
                        </tr>
                      ))}
                    </tbody>
              ...
              <div className="bill-summary-card">
                <div className="summary-line"><span>Subtotal</span><span>Rs.{billSubtotal.toFixed(2)}</span></div>
                <div className="summary-line"><span>GST (5%)</span><span>Rs.{billTax.toFixed(2)}</span></div>
                <div className="summary-line grand-total"><span>Grand Total</span><span style={{ color: 'var(--foody-orange)' }}>Rs.{billGrandTotal.toFixed(2)}</span></div>

                <div style={{ marginTop: '12px', display: 'flex', gap: '8px' }}>
                  {['UPI / GPay', 'Card', 'Cash'].map(mode => (
                    <button key={mode} className={`payment-mode-pill ${paymentMethod === mode ? 'active' : ''}`} onClick={() => setPaymentMethod(mode)}>{mode}</button>
                  ))}
                </div>

                <button onClick={handleProceedToPay} disabled={billItems.length === 0 || isCheckingOut} className="btn-proceed-pay">
                  {isCheckingOut ? (
                    <><RefreshCw className="spinner" size={18} /><span>Processing & Updating CSV...</span></>
                  ) : (
                    <><CreditCard size={18} /><span>Proceed to Pay (Rs.{billGrandTotal.toFixed(2)})</span><ArrowRight size={18} /></>
                  )}
                </button>
                <p style={{ fontSize: '0.72rem', color: 'var(--text-muted)', textAlign: 'center', marginTop: '6px' }}>
                  Stock will be decremented in Data/inventory.csv upon payment.
                </p>
              </div>
            </div>
          </div>
                          ...
                          {group.items.map((p) => (
                            <div key={p.product_id} className="clean-product-card">
                              ...
                              <div className="card-info">
                                ...
                                <span className="card-pack-unit">Package: {p.unit_of_measure}</span>
                                {p.barcode && <span className="card-barcode-code">{p.barcode}</span>}
                              </div>
